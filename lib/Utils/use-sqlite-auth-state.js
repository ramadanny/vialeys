...
// Module loading for better-sqlite3
let Database;
let loadError = null;
...
const useSQLiteAuthState = async (dbPath) => {
    ...
    if (loadError) {
        throw new Error(
            `Failed to load 'better-sqlite3': ${loadError.message}\n\n` +
            `To use the SQLite authentication state, please ensure the following:\n` +
            `1. The package is installed ('npm install better-sqlite3' or 'yarn add better-sqlite3').\n` +
            `2. Your system has the required native build tools (Python, Make, and a C++ compiler) to compile the module.\n` +
            `3. You are running this application using Node.js. (Note: The Bun runtime is not supported for this specific feature).`
        );
    }
