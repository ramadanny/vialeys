...
const { getDevice, getContentType, normalizeMessageContent } = require('../Utils')
const {
  areJidsSameUser,
  isJidBroadcast,
...
const { aesDecryptGCM, hmacSign } = require('./crypto')
const { getKeyAuthor, toNumber } = require('./generics')
  ...
  if (protocolMsg) {
    ...
    const emitGroupUpdate = update => {
      ev.emit('groups.update', [
        { id: jid, ...update, author: message.participant ? message.participant : undefined }
      ])
    }
