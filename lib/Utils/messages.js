...
const prepareWAMessageMedia = async (message, options) => {
  ...
  if (isNewsletter) {
    ...
    const { mediaUrl: mediaUrl, directPath: directPath } = await options.upload(
      filePath,
      {
...
const prepareAlbumMessageContent = async (jid, albums, options) => {
  ...
  if (albums.length === 0) {
    throw new Error(
      "albums cannot be empty. At least one media item is required.",
    );
  }
  const validCount = albums.filter((m) => "image" in m || "video" in m).length;
  if (validCount === 0) {
    throw new Error(
      "albums contains no valid media. Use 'image' or 'video' keys.",
    );
  }
  let mediaHandle;
  let mediaMsg;
...
const extractMessageContent = (content) => {
  const extractFromButtonsMessage = (msg) => {
    ...
    if (header ? msg.header?.imageMessage : msg.imageMessage) {
      ...
      return {
        documentMessage: header
          ? msg.header.documentMessage
          : msg.documentMessage,
      };
      ...
      return {
        locationMessage: header
          ? msg.header.locationMessage
          : msg.locationMessage,
      };
  ...
  if (content?.templateMessage?.interactiveMessageTemplate) {
    return extractFromButtonsMessage(
...
function getAggregateVotesInPollMessage(
  { message: message, pollUpdates: pollUpdates },
  meId,
) {
...
function getAggregateResponsesInEventMessage(
  ...
  for (const update of eventResponses) {
    ...
    const responseType =
      proto.Message.EventResponseMessage.EventResponseType[response];
