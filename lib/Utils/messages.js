...
const { URL_REGEX, WA_DEFAULT_EPHEMERAL } = require('../Defaults/constants')
const { MEDIA_KEYS } = require('../Defaults/media')
const { WAProto, WAMessageStatus } = require('../Types')
const {
  isJidGroup,
  isJidNewsletter,
...
const { generateMessageID, getKeyAuthor, unixTimestampSeconds } = require('./generics')
const {
...
const generateWAMessageContent = async (message, options) => {
  ...
  if ('sections' in message && !!message.sections) {
    ...
    m = { interactiveMessage }
  } else if ('bottomSheet' in message) {
    const sheetConfig = message.bottomSheet
    const paramsJson = {
      bottom_sheet: {
        in_thread_buttons_limit: sheetConfig.displayLimit || 2,
        divider_indices: sheetConfig.dividers || [1, 2, 3, 4, 5, 999],
        list_title: sheetConfig.listTitle || '\0',
        button_title: sheetConfig.buttonTitle || 'Click Here'
      }
    }

    if (sheetConfig.offer) {
      paramsJson.limited_time_offer = sheetConfig.offer
    }

    const interactiveMessage = {
      nativeFlowMessage: {
        buttons: sheetConfig.buttons || [],
        messageParamsJson: JSON.stringify(paramsJson)
      }
    }
    ...
    if (message.image) {
      const media = await prepareWAMessageMedia({ image: message.image }, options)
      Object.assign(header, { ...media, hasMediaAttachment: true })
    } else if (message.video) {
      const media = await prepareWAMessageMedia({ video: message.video }, options)
      Object.assign(header, { ...media, hasMediaAttachment: true })
      ...
      header.locationMessage = message.location
    ...
    if ('text' in message) {
      ...
      if ('caption' in message) {
        ...
        interactiveMessage.body = { text: message.body }
      } else {
...
const generateWAMessage = async (jid, content, options) => {
  ...
  return generateWAMessageFromContent(
    jid,
    await generateWAMessageContent(content, { newsletter: isJidNewsletter(jid), ...options }),
    options
  )
}
