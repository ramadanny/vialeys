...
const { KEY_BUNDLE_TYPE, MIN_PREKEY_COUNT, DEFAULT_CACHE_TTLS } = require('../Defaults/constants')
const {
  XWAPaths,
  XWAPathsMexUpdates,
  MexOperations,
  ...
  const handleGroupNotification = (participant, child, msg, mode) => {
    ...
    switch (child.tag) {
      ...
      case 'modify':
        const oldNumber =
          mode === 'lid'
            ? getBinaryNodeChildren(child, 'participant').map(p => p.attrs.phone_number)
            : getBinaryNodeChildren(child, 'participant').map(p => p.attrs.jid)
      ...
      case 'created_membership_requests':
        participantJid =
          mode === 'lid'
            ? getBinaryNodeChild(child, 'requested_user')?.attrs?.phone_number
            : getBinaryNodeChild(child, 'requested_user')?.attrs?.jid || participant
      ...
      case 'unlink':
        ...
        const groups = getBinaryNodeChildren(child, 'group')
          ...
          .filter(x => x)
        msg.messageStubType =
          stubMap?.[type] || WAMessageStubType[`COMMUNITY_${child.tag.toUpperCase()}_PARENT_GROUP`]
    ...
    switch (nodeType) {
      ...
      case 'w:gp2':
        ...
        handleGroupNotification(
          mode === 'lid' ? node.attrs.participant_pn : node.attrs.participant,
          child,
          result,
          mode
        )
      ...
      case 'picture':
        ...
        ev.emit('contacts.update', [
          {
            id:
              jidNormalizedUser(node?.attrs?.from) || (setPicture || delPicture)?.attrs?.hash || '',
