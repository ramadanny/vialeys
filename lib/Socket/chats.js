...
} = require("../Defaults/history");
const {
  SyncState: SyncState,
  ALL_WA_PATCH_NAMES: ALL_WA_PATCH_NAMES,
} = require("../Types");
const {
...
const makeChatsSocket = (config) => {
  ...
  const addOrEditContact = (jid, contact) =>
    chatModify({ contact: contact }, jid);
  const removeContact = (jid) => chatModify({ contact: null }, jid);
  const addLabel = (jid, labels) =>
    chatModify({ addLabel: { ...labels } }, jid);
  const addChatLabel = (jid, labelId) =>
    chatModify({ addChatLabel: { labelId: labelId } }, jid);
  ...
  const addOrEditQuickReply = (quickReply) =>
    chatModify({ quickReply: quickReply }, "");
  ...
  ws.on("CB:ib,,dirty", async (node) => {
    ...
    switch (type) {
      case "account_sync":
        if (attrs.timestamp) {
          let { lastAccountSyncTimestamp: lastAccountSyncTimestamp } =
            authState.creds;
