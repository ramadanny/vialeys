...
const makeSocket = (config) => {
  ...
  const rotateSignedPreKey = async () => {
    ...
    await query({
      ...
      content: [
        { tag: "rotate", attrs: {}, content: [xmppSignedPreKey(skey)] },
      ],
    });
  ...
  const uploadPreKeys = async (count = MIN_PREKEY_COUNT, retryCount = 0) => {
    ...
    const uploadLogic = async () => {
      logger.info(
        { count: count, retryCount: retryCount },
        "uploading pre-keys",
      );
  ...
  ws.on("CB:iq,,pair-success", async (stanza) => {
    ...
    try {
      const { reply: reply, creds: updatedCreds } = configureSuccessfulPairing(
        stanza,
        creds,
      );
      logger.info(
  ...
  ws.on("CB:stream:error", (node) => {
    ...
    logger.error(
      { reasonNode: reasonNode, fullErrorNode: node },
      "stream errored out",
    );
    const { reason: reason, statusCode: statusCode } =
