...
const {
  makeMessagesRecvSocket: makeMessagesRecvSocket,
} = require("./messages-recv");
const makeBusinessSocket = (config) => {
  ...
  const updateCoverPhoto = async (photo) => {
    const { fileSha256: fileSha256, filePath: filePath } =
      await getRawMediaUploadData(photo, "biz-cover-photo");
