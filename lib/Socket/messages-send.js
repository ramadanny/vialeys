...
const { DEFAULT_CACHE_TTLS, WA_DEFAULT_EPHEMERAL } = require('../Defaults/constants')
const {
  delay,
  assertMediaContent,
  bindWaitForEvent,
...
const { QueryIds, XWAPaths, WAMessageAddressingMode } = require('../Types')
const {
  areJidsSameUser,
  getBinaryNodeChild,
    ...
    const userEncryptionPromises = Array.from(devicesByUser.entries()).map(([user, userDevices]) =>
      encryptionMutex.mutex(user, async () => {
        ...
        const getEncryptionJid = async wireJid => {
          if (!isJidUser(wireJid)) return wireJid
          ...
          try {
            ...
            try {
              const migrationResult = await signalRepository.migrateSession(
                [wireJid],
                lidWithDevice
              )
    ...
    await authState.keys.transaction(async () => {
      ...
      if (messages.interactiveResponseMessage?.nativeFlowResponseMessage) {
        extraAttrs['native_flow_name'] =
          messages.interactiveResponseMessage.nativeFlowResponseMessage?.name || 'menu_options'
      }
        ...
        if (!participant) {
          ...
          if (!isStatus) {
            const groupAddressingMode =
              ...
              (isLid ? WAMessageAddressingMode.LID : WAMessageAddressingMode.PN)
            additionalAttributes = {
              ...additionalAttributes,
