...
} = require("../Defaults/connection");
const {
  LabelAssociationType: LabelAssociationType,
} = require("../Types/LabelAssociation");
const {
  md5: md5,
  toNumber: toNumber,
...
const {
  makeOrderedDictionary: makeOrderedDictionary,
} = require("./make-ordered-dictionary");
...
const makeInMemoryStore = (config) => {
    ...
    ev.on("contacts.update", async (updates) => {
      for (const update of updates) {
        ...
        if (contacts[update.id]) {
          ...
          contact =
            contacts[contactHashes.find(([, b]) => b === update.id?.[0]) || ""];
        }
        if (contact) {
  ...
  return {
    ...
    getChatLabels: (chatId) =>
      labelAssociations.filter((la) => la.chatId === chatId).all(),
