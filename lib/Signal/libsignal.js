...
const { SessionCipher, SessionBuilder, SessionRecord, ProtocolAddress } = require('libsignal-node')
const { generateSignalPubKey } = require('../Utils')
const { jidDecode, transferDevice } = require('../WABinary')
    ...
    async encryptMessage({ jid, data }) {
      ...
      if (jid.includes('@s.whatsapp.net')) {
        ...
        if (lidForPN?.includes('@lid')) {
          ...
          const { [lidAddr.toString()]: lidSession } = await auth.keys.get('session', [
            lidAddr.toString()
          ])
          if (lidSession) {
            ...
            const { [pnAddr.toString()]: pnSession } = await auth.keys.get('session', [
              pnAddr.toString()
            ])
            if (pnSession) {
              // Migrate PN to LID
    ...
    loadSession: async id => {
      try {
        ...
        if (id.includes('.') && !id.includes('_1')) {
          ...
          const pnJid =
            device === '0' ? `${parts[0]}@s.whatsapp.net` : `${parts[0]}:${device}@s.whatsapp.net`
